...
class LagouCrawler:
    ...
    def save_csv(self, df, p):
        # 保存文件
        df.to_csv(path_or_buf='d:\\lagou_%d.csv' % p, encoding='gbk')
    ...
    def crawler(self):
        ...
        for p in range(self.pages + 1):
            ...
            for row in position_result:
                tmp_list.append([row['positionName'], row['workYear'], row['education'], row['financeStage'],
                                 row['companyShortName'], row['industryField'], \
                                 row['city'], row['salary'], row['positionId'], row['positionAdvantage'],
                                 row['district'], ','.join(row['positionLables']), \
                                 ','.join(row['companyLabelList']), row['companySize'], row['companyFullName']])
            df = pd.DataFrame(tmp_list,
                              columns=['position_name', 'work_year', 'education', 'finance_stage', 'company_short_name',
                                       'industry_field', \
                                       'city', 'salary', 'position_id', 'position_advantage', 'district',
                                       'position_lables', \
